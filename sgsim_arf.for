      INCLUDE 'gslib/acorni.f'
      INCLUDE 'gslib/backtr.f'
      INCLUDE 'gslib/chknam.f'
      INCLUDE 'gslib/cova3.f'
      INCLUDE 'gslib/locate.f'
      INCLUDE 'gslib/powint.f'
      INCLUDE 'gslib/gauinv.f'
      INCLUDE 'gslib/getindx.f'
      INCLUDE 'gslib/setrot.f'
      INCLUDE 'gslib/setsupr.f' !*
      INCLUDE 'gslib/srchsupr.f'
      INCLUDE 'gslib/sqdist.f'
      INCLUDE 'gslib/ksol.f'
      INCLUDE 'gslib/gcum.f'
      INCLUDE 'gslib/picksupr.f'
      INCLUDE 'gslib/sortem.f' !*
c
...
c
 ...
 9997       format(' nst must be at least 1, it has been set to ',i4,/,
     ...
     +                   ang1(i),ang2(i),ang3(i)
      ...
      read(lin,*,err=98) iarf
      write(*,*) ' Antithetic Random Fields Generated=',iarf
      write(*,*)
      close(lin)
c
...
c
      allocate(cor(iarf,iarf),stat = test)
            if(test.ne.0)then
...
c
      allocate(inf(iarf,iarf),stat = test)
            if(test.ne.0)then
                  write(*,*)'ERROR 32: Allocation failed',
     +                  ' due to insufficient memory.'
                  stop
...
c
      allocate(rmatrix(MXYZ,iarf),stat = test)
            if(test.ne.0)then
                  write(*,*)'ERROR 33: Allocation failed',
     +                  ' due to insufficient memory.'
                  stop
            end if
c
      allocate(izsbtosr(8*MAXSB),stat = test)
            if(test.ne.0)then
                  write(*,*)'ERROR 34: Allocation failed',
     +                  ' due to insufficient memory.'
                  stop
            end if
c
...
c
c Generate the correlation Matrix
c
      ...
      do ir=1,iarf
         do ic=1,iarf
	         if (ir.eq.ic) then
		         ...
		         cor(ir,ic) = -(1/REAL(iarf-1))+0.0000001
	         END IF
         END DO
      END DO
      Write (*,*) 'The alpha coefficient is:', cor(1,2)
      call chol(cor,inf,iarf,iarf,ierr)
       indicator = iarf
	     ...
	     Do while (procede==1)
	         do i=1,nxyz
	             DO j=1,iarf
	                 ...
	                 aux1(j)=xp
	                 END DO
	         aux2=matmul(inf,aux1)
	         DO k=1,iarf
		     rmatrix(i,k)=aux2(k)
	             END DO
                 END DO
		    maximo=real(maxval(rmatrix))
		    minimo=real(minval(rmatrix))
		    Open (unit=15, file="random_max.txt")
		    write(15,*), maximo,minimo
		    close(15)
		    IF ((maximo <= 100).AND.(minimo >= -100)) THEN 
		         procede=0
		         end IF
	     END DO
	     indicator=0

c      Open(unit=15,file="random_numbers.txt")
c      do i=1,iarf
c         Write(15,*),'simulación:',i
c	     Do j=1,nxyz
c	     Write(15,*), rmatrix(j,i)
c	     End DO
c      END DO
      CLOSE(15)
         WRITE(*,*) 'Max Value: ', maximo
         WRITE(*,*) 'Min Value: ', minimo
         END IF
...
c
                  p = rmatrix(in,indicator)
                  call gauinv(p,xp,ierr)
                  sim(index) = p * cstdev + cmean
...
c-----------------------------------------------------------------------
 ...
 43   format('         10.0  10.0  10.0     ',
     +       '-a_hmax, a_hmin, a_vert')
      write(lun,44)
 44   format('2 							',
     +       '-Antitetic Random Fields Generated')
      close(lun)
      return
      end
